...
{
    gpio_pad_select_gpio(MODRXorTX_GPIO);
    gpio_set_direction(MODRXorTX_GPIO, GPIO_MODE_OUTPUT);
...
void uart2_rx_task(void *arg)
{
    ...
    {
        memset(&senddata,0,sizeof(_serialbuf_st));
        serialbuf.len = uart_read_bytes(UART_NUM_2, serialbuf.buf, RX_BUF_SIZE, 10 / portTICK_RATE_MS);
        if (serialbuf.len>0)
        {
            //ESP_LOGI("serial  ", "len %d", serialbuf.len);
            esp_log_buffer_hex("serial  ", serialbuf.buf, serialbuf.len);
